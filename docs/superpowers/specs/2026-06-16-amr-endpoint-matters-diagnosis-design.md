...
| 支柱 | 一句话 | 落到本 spec |
|---|---|---|
| **关系智能** | 人事和谐 | Block 1 — Endpoint 路由模型 |
...
- **rubric / 错位词典 = 可编辑本地文件**（`~/.config/jl/diagnosis_rubric.md`，路径走
  `AMR_DIAGNOSIS_RUBRIC`，**内容留本地、不进公开仓库**，同打法库机制）。内容：外圆内方
  (儒圆 礼恕仁 / 法方 对事不对人 / 墨力 执行给确定) + 错位6型(圆缺/圆过/方污/方先/力先/力无)
  及其修法 + 伦理护栏(真心校验：辅助真心✅ / 制造假暖❌；诚实>漂亮)。
- **结构化诊断对象**（结构是脚手架，nuance 在自由文本）：
  `{圆: ok|缺|过, 方: ok|污|先, 力: ok|先|无, 错位: <enum|''>, 真心: ok|假暖, 一句话诊断: str, 口径: str}`。
