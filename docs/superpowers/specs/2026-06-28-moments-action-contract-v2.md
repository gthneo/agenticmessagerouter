...
> **真相源** AMR 仓（本文件）—— AMR 定义契约，fullwechat 实现。fullwechat 草案见 `~/as/fullwechet/docs/moments-action-contract-v2-publish.md`（本文件是其评审定稿版，以本文件为准）。
> **关系** v1（`2026-06-28-moments-action-contract-v1.md`）= 朋友圈**读 + 点赞**，**仍然有效**；本 v2 **只新增「发布（publish / 一对多广播）」**，不重述读/点赞（DRY）。
> **姊妹契约** message-canonical / send-target / group-metadata（同日，AMR 仓）。
